...
Процедура Главная()
	...
	ИмяФайла = "C:\Users\Anny_USER\Documents\КонсольКодаУФ.epf"; //Укажите имя файла
	//Если не найден раздел или пользователь, будет выдано сообщение, раздел или пользователь будет пропущен
	Разделы = "Администрирование (Подсистема), Продажи (Подсистема)"; //Через запятую, можно с пробелами 
	Пользователи = "Атрахимович.О.А, Ачаева"; //Через запятую, можно с пробелами
	
	//Остальной код не меняется...
	
	РазделыМассив = СтрРазделить(Разделы, ",", ложь);
	ПользователиМассив = СтрРазделить(Пользователи, ",", ложь);
	...
	//Добавляем разделы
	Для Каждого Раздел ИЗ РазделыМассив Цикл
		РазделСсылка = v8.Справочники.ИдентификаторыОбъектовМетаданных.НайтиПоНаименованию(СокрЛП(Раздел), Истина);
		Если Не ЗначениеЗаполнено(РазделСсылка) Тогда
			Сообщить("Не найден раздел: " + Раздел);
		Иначе
			НСтр = ОО.Разделы.Добавить();
			НСтр.Раздел = РазделСсылка;
		КонецЕсли;
	КонецЦикла;
	
	ОО.Записать(); ДопСсылка = ОО.Ссылка;
	
	НЗ = v8.РегистрыСведений.ПользовательскиеНастройкиДоступаКОбработкам.СоздатьНаборЗаписей();
	НЗ.Отбор.ДополнительныйОтчетИлиОбработка.Значение = ДопСсылка;
	НЗ.Отбор.ДополнительныйОтчетИлиОбработка.Использование = Истина;
	
	//Добавляем разделы
	Для Каждого Пользователь ИЗ ПользователиМассив Цикл
		ПользовательСсылка = v8.Справочники.Пользователи.НайтиПоНаименованию(СокрЛП(Пользователь), Истина);
		Если Не ЗначениеЗаполнено(ПользовательСсылка) Тогда
			Сообщить("Не найден Пользователь: " + Пользователь);
		Иначе
			Для Каждого Строка ИЗ ОО.Команды Цикл
				МЗ = НЗ.Добавить();
				МЗ.ДополнительныйОтчетИлиОбработка = ДопСсылка;
				МЗ.ИдентификаторКоманды = Строка.Идентификатор;
				МЗ.Пользователь = ПользовательСсылка;
				МЗ.Доступно = Истина;
			КонецЦикла;
		КонецЕсли;
	КонецЦикла;
